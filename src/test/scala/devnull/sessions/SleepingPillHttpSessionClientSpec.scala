...
class SleepingPillHttpSessionClientSpec extends FunSpec with Matchers {
  ...
  describe("SleepingPillHttpSessionClient") {
    ...
    it("should be able to fetch session") {
      val eventId = UuidFromString("30d5c2f1cb214fc8b0649a44fdf3b4bf")
        .map(EventId.apply)
        .right
        .get
      val sessionId = UuidFromString("32e6aca94d934fc1bc05073c68bc681e")
        .map(SessionId.apply)
        .right
        .get
    ...
    ignore("should be able to fetch session for new api aaa") {
      ...
      val sessionId =
        SessionId(UUID.fromString("31d7e476-662f-48f8-bcdf-cac88d9ff742"),
                  false)
