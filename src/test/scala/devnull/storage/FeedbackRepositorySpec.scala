  ...
  describe("Postgres database") {
    ...
    it("should insert a feedback", DatabaseTag) {
      val response: FeedbackId = repo
        .insertFeedback(FeedbackTestData.createFeedback())
        .transact(xa)
        .unsafePerformSync
    ...
    it("should query an inserted feedback", DatabaseTag) {
      ...
      val feedbacks: List[Feedback] =
        repo.selectFeedbacks().transact(xa).unsafePerformSync
    ...
    it("should get average of session feedback") {
        ...
        r1_ignored <- repo
                       .insertFeedback(
                         Feedback(
                           ...
                           info_1,
                           sessionId,
                           Ratings(2, Some(3), Some(4), Some(5))
                         )
                       )
                       .transact(xa)
        r1 <- repo
        ...
        r2 <- repo
               .insertFeedback(
                 Feedback(
                   ...
                   info_2,
                   sessionId,
                   Ratings(5, Some(4), Some(3), Some(2))
                 )
               )
               .transact(xa)
      ...
      val result: Option[FeedbackResult] =
        repo.selectFeedbackForSession(sessionId).transact(xa).unsafePerformSync
    ...
    it("should get average of event feedback") {
      ...
      val insert = for {
        ...
        t2 <- paperRepo
               ...
               .transact(xa)
        t3 <- repo
               .insertFeedback(FeedbackTestData.createFeedback(sessionIdOne))
               .transact(xa)
        t4 <- repo
               .insertFeedback(FeedbackTestData.createFeedback(sessionIdOne))
               .transact(xa)
        t5 <- repo
               .insertFeedback(FeedbackTestData.createFeedback(sessionIdTwo))
               .transact(xa)
        ...
        t6 <- paperRepo
               ...
               .transact(xa)
        t7 <- repo
               .insertFeedback(FeedbackTestData.createFeedback(sessionIdThree))
               .transact(xa)

        t8 <- repo
               .insertFeedback(FeedbackTestData.createFeedback(voterId = "12345"))
               .transact(xa)
      } yield "done"
    ...
    describe("comments") {
      ...
      it("should return empty list when no result ", DatabaseTag) {
        ...
        repo
          .insertFeedback(
            FeedbackTestData.createFeedback(session = sessionId, comments = None)
          )
          .transact(xa)
          .unsafePerformSync

        val comments: List[String] =
          repo.selectComments(sessionId).transact(xa).unsafePerformSync
      ...
      it("should only show comment for the given session", DatabaseTag) {
        ...
        repo
          .insertFeedback(
            FeedbackTestData
              .createFeedback(session = sessionId, comments = Some("Session one"))
          )
          .transact(xa)
          .unsafePerformSync
        repo
          .insertFeedback(
            FeedbackTestData
              .createFeedback(session = UUID.randomUUID(), comments = Some("Session two"))
          )
          .transact(xa)
          .unsafePerformSync
      ...
      it("should return the last comment for a [session, voterId]", DatabaseTag) {
        ...
        repo
          .insertFeedback(
            FeedbackTestData
              .createFeedback(session = sessionId, comments = Some("Fist comment"))
          )
          .transact(xa)
          .unsafePerformSync
        repo
          .insertFeedback(
            FeedbackTestData
              .createFeedback(session = sessionId, comments = Some("Last comment"))
          )
          .transact(xa)
          .unsafePerformSync
      ...
      it("should return list of comments for unique votes", DatabaseTag) {
        ...
        repo
          .insertFeedback(
            FeedbackTestData.createFeedback(
              session = sessionId,
              voterId = "1234",
              comments = Some("One")
            )
